...
/** DeploymentProfile — GitW3 publishes one per deployment, world-readable. */
const DEPLOYMENT_PROFILE_ONTOLOGY = "d38e0c5b-9d63-4a21-8e8b-1d6b63af64d2";
...
/**
 * Deployments published for one platform release. Evidence for the deployment
 * assurance dimension: a deployment profile means the software actually
 * running was tied back to a reviewed release, rather than merely declared.
 */
export async function listDeployments(
    ...
    let packets: Packet[];
    try {
        packets = await all({ ontology: DEPLOYMENT_PROFILE_ONTOLOGY });
    } catch (error) {
        console.error("[ppa/aaas] failed loading deployments:", error);
        return [];
    }

    const byDeployment = new Map<string, DeploymentRecord>();
    for (const packet of packets) {
        const data = packet.data;
        if (!data) continue;
        if (str(data.platformEname) !== platformEName) continue;
        if (str(data.version) !== version) continue;
        const ename = str(data.deploymentEname);
        if (!ename) continue;
        byDeployment.set(ename, {
            deploymentEName: ename,
            platformEName: str(data.platformEname),
            version: str(data.version),
