 ...
 */
async function createGroupEVault(
    registryUrl,
    provisionerUrl,
    groupData,
    verificationCode,
...
async function createGroupManifestWithRetry(
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            const { GraphQLClient } = await Promise.resolve().then(() =>
                __importStar(require("graphql-request")),
            );
