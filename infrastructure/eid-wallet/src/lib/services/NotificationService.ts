...
class NotificationService {
  ...
  async registerDevice(eName: string): Promise<boolean> {
    try {
      ...
      const response = await fetch(`${this.provisionerEndpoint}/api/devices/register`, {
        method: 'POST',
        headers: {
  ...
  async unregisterDevice(): Promise<boolean> {
    ...
    try {
      const response = await fetch(`${this.provisionerEndpoint}/api/devices/unregister`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  async checkAndShowNotifications(): Promise<void> {
    try {
      ...
      if (!registration) {
        console.log('No device registration found, attempting to register...');
        
        // Try to get eName from vault
        try {
          const vault = await this.getVaultEname();
          if (vault?.ename) {
            console.log(`Found eName: ${vault.ename}, registering device...`);
            const registered = await this.registerDevice(vault.ename);
            if (registered) {
            ...
            console.log('No eName found in vault, skipping notification check');
            return;
          }
        } catch (error) {
      ...
      if (!registration) {
        console.log('Still no device registration, skipping notification check');
        return;
      }

      // Check for notifications from provisioner
      const response = await fetch(`${this.provisionerEndpoint}/api/notifications/check`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        ...
        console.log('No notifications endpoint available or error:', response.status);
      }
    } catch (error) {
  ...
  private async getVaultEname(): Promise<{ ename: string } | null> {
    try {
      ...
      const store = await import('@tauri-apps/plugin-store').then(m => m.Store.load('global-state.json'));
