            ...
            try {
                ...
                if (data.type === "auth") {
                    ...
                    if (!data.redirect || typeof data.redirect !== "string") {
                        console.error("Invalid redirect URL:", data.redirect);
...
function handleSigningRequest(content: string) {
    try {
        ...
        if (content.startsWith("w3ds://")) {
            parseableContent = content.replace("w3ds://", "https://dummy.com/");
        }
        ...
        try {
            ...
            console.log("🔍 DEBUG: Data keys:", Object.keys(signingData || {}));
            console.log(
...
async function handleBlindVotingRequest(
    ...
    try {
        ...
        const pollResponse = await fetch(`${platformUrl}/api/polls/${pollId}`);
        if (!pollResponse.ok) {
        ...
        console.log("  - signingData.pollDetails:", !!signingData?.pollDetails);
    } catch (error) {
...
async function handleBlindVote() {
    ...
    console.log("🔍 DEBUG: selectedBlindVoteOption:", selectedBlindVoteOption);
    ...
    try {
        ...
        console.log("🔗 Submitting blind vote to API:", signingData.redirect);
        ...
        console.log("🔍 DEBUG: signingData.redirect:", signingData.redirect);
        console.log(
        ...
        if (response.status >= 200 && response.status < 300) {
            ...
            blindVoteError = "Failed to submit blind vote. Please try again.";
        }
    } catch (error) {
...
async function handleRevealVote() {
    ...
    try {
        ...
        console.log("🔍 Debug: storedVoteData type:", typeof storedVoteData);
        console.log("🔍 Debug: storedVoteData length:", storedVoteData?.length);
        ...
        try {
            ...
            console.log("🔍 Debug: Final revealedVoteData:", revealedVoteData);
            revealSuccess = true;
                ...
                try {
                    ...
                    if (firstBrace !== -1 && lastBrace !== -1) {
                        console.log(
                            ...
                            storedVoteData.substring(firstBrace, lastBrace + 1),
                        );
                    }
                } catch (debugError) {
