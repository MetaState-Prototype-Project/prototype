...
import { z } from "zod";
import { ArrowLeft, Save, Plus, Instagram, Facebook, MessageCircle, Check, ChevronsUpDown } from "lucide-react";
...
export default function CreateCharter() {
  ...
  const createCharterMutation = useMutation({
    mutationFn: async (data: CreateCharterForm) => {
      const response = await apiRequest("POST", "/api/charters", data);
      return response.json();
    },
    onSuccess: () => {
      toast({
        title: "Charter created successfully",
        description: "Your new charter has been created and is now active.",
      });
    ...
    onError: (error) => {
      ...
      toast({
        title: "Error creating charter",
        description: "There was an error creating your charter. Please try again.",
        variant: "destructive",
      });
    },
  });
  ...
  const onSubmit = async (data: CreateCharterForm) => {
    ...
    try {
      await createCharterMutation.mutateAsync(finalData);
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="animate-pulse space-y-6">
          <div className="h-8 bg-black bg-opacity-10 rounded w-1/4"></div>
          <div className="h-96 bg-black bg-opacity-10 rounded-3xl"></div>
        </div>
      </div>
    );
  }
      ...
      <Card className="bg-white/70 backdrop-blur-xs rounded-3xl soft-shadow">
        <CardContent className="p-4 sm:p-6 lg:p-8">
          <div className="mb-6 sm:mb-8">
            ...
            <p className="text-gray-600 text-sm sm:text-base">Set up community guidelines and structure for your social media group</p>
          </div>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 sm:space-y-8">
                                  ...
                                  <div className="flex items-center space-x-3 w-full">
                                    ...
                                    <div className="flex-1">
                                      ...
                                      <div className="flex items-center space-x-2 text-sm text-gray-500">
                                        <PlatformBadge platform={group.platform} className="text-xs" />
              ...
              <div>
                <Label className="text-sm font-medium text-gray-700 mb-3 block">Community Guidelines</Label>
                <div className="space-y-3">
                  {guidelines.map((guideline, index) => (
                    <div key={index} className="flex items-start space-x-3">
                      <div className="w-6 h-6 rounded-lg bg-amber-100 flex items-center justify-center mt-1">
                        <span className="text-amber-600 font-medium text-sm">{index + 1}</span>
                      </div>
                      <Input
              ...
              <div className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-4 pt-4 sm:pt-6">
                ...
                >
                  ...
                  {createCharterMutation.isPending ? "Creating Charter..." : "Create Charter"}
                </Button>
              </div>
            </form>
          </Form>
