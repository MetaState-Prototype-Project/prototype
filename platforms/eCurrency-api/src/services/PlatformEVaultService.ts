...
export class PlatformEVaultService {
    ...
    /**
     * Check if eCurrency platform eVault already exists
     */
    async checkPlatformEVaultExists(): Promise<boolean> {
        ...
        const existingMapping = await mappingRepository.findOne({
            where: { localUserId: "ecurrency-platform" },
        });
        return !!existingMapping;
    }

    /**
     * Create eVault for eCurrency platform (one-time setup)
     */
    async createPlatformEVault(): Promise<{
        ...
        if (exists) {
            throw new Error("Platform eVault already exists for eCurrency");
        }
        ...
        try {
            ...
            mapping.localUserId = "ecurrency-platform";
            ...
            mapping.userProfileData = {
                ...
                displayName: "eCurrency Platform",
                description:
