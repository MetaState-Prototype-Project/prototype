	...
	onMount(() => {
		const handleDeeplinkLogin = async () => {
			try {
				...
				const apiBaseUrl = import.meta.env.VITE_FILE_MANAGER_BASE_URL;
				...
				if (response.ok) {
					...
					if (data.token && data.user) {
						localStorage.setItem("file_manager_auth_token", data.token);
						localStorage.setItem("file_manager_user", JSON.stringify(data.user));
