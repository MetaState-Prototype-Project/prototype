...
export class PlatformEVaultService {
    ...
    /**
     * Check if File Manager platform eVault already exists
     */
    async checkPlatformEVaultExists(): Promise<boolean> {
        ...
        const existingMapping = await mappingRepository.findOne({
            where: { localUserId: "file-manager-platform" },
        });
        return !!existingMapping;
    }

    /**
     * Create eVault for File Manager platform (one-time setup)
     */
    async createPlatformEVault(): Promise<{
        ...
        if (exists) {
            throw new Error("Platform eVault already exists for File Manager");
        }
        ...
        try {
            ...
            mapping.localUserId = "file-manager-platform";
            ...
            mapping.userProfileData = {
                ...
                displayName: "File Manager Platform",
                description:
