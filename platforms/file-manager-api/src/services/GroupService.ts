...
export class GroupService {
    ...
    async createGroup(
        ...
        // For File Manager Chat groups, use a transaction to prevent race conditions
        if (isPrivate && (name.startsWith("File Manager Chat") || name.includes("File Manager Chat")) && memberIds.length === 2) {
            return await AppDataSource.transaction(async (transactionalEntityManager) => {
                // First check by description pattern (idempotency check using :: pattern)
                if (description && description.includes("::")) {
                    const existingByDescription = await transactionalEntityManager.findOne(Group, {
                        where: {
                            name: name,
                    ...
                    if (existingByDescription) {
                        console.log(`⚠️ DM already exists with description pattern, returning existing DM: ${existingByDescription.id}`);
                        return existingByDescription;
                    }
                }
