<script lang="ts">
	...
	let { entries = [], editable = false }: { entries?: Education[]; editable?: boolean } = $props();

	let showForm = $state(false);
	let editIndex = $state<number | null>(null);
	let form = $state<Education>({ institution: '', degree: '', startDate: '', sortOrder: 0 });

	function openAdd() {
		form = { institution: '', degree: '', fieldOfStudy: '', startDate: '', endDate: '', description: '', sortOrder: entries.length };
		editIndex = null;
		showForm = true;
	}
			...
			<Card class="mb-4 bg-muted/30">
				<CardContent class="grid gap-3 pt-6">
					<div class="space-y-1">
						...
						<Input bind:value={form.institution} placeholder="e.g. ETH Zurich" />
					</div>
					<div class="space-y-1">
						<Label>Degree</Label>
						<Input bind:value={form.degree} placeholder="e.g. BSc Computer Science" />
					</div>
					<div class="space-y-1">
						<Label>Field of study</Label>
						<Input bind:value={form.fieldOfStudy} placeholder="e.g. Computer Science" />
					</div>
					...
					<div class="space-y-1">
						...
						<Textarea bind:value={form.description} rows={3} placeholder="Activities, achievements..." />
					</div>
		...
		<div class="space-y-4">
			{#each entries as entry, i}
				...
				<div class="flex items-start justify-between">
					<div>
						<h3 class="text-sm font-semibold text-foreground">{entry.degree}{entry.fieldOfStudy ? `, ${entry.fieldOfStudy}` : ''}</h3>
			...
			{:else}
				{#if !showForm}
					<p class="text-sm italic text-muted-foreground/50">{editable ? 'Add your education' : 'No education listed'}</p>
				{/if}
			{/each}
		</div>
	</CardContent>
