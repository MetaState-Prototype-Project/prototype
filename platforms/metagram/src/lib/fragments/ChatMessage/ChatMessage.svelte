...
<div {...restProps} class={cn([`flex items-start gap-2 ${transactionType === "incoming" ? "flex-row-reverse" : "flex"}`, restProps.class].join(" "))}>
    {#if isHeadNeeded}
	<Avatar size="xs" src={userImgSrc} />
    {/if}
	<div
		class={cn(
			`w-[max-content] max-w-[50%]  ${isHeadNeeded ? "translate-y-4" : "translate-y-0"}  ${transactionType === 'incoming' ? 'translate-x-[-8px]' : 'translate-x-2'}`
		)}
	>
		<div
			class={cn(
				`relaitve rounded-3xl px-4 py-2 ${transactionType === 'incoming' ? 'bg-brand-burnt-orange' : 'bg-grey'}`
			)}
		>
            {#if isHeadNeeded}
			<svg
		...
		<p
			class={cn(
				`subtext text-black-400 mt-0.5 flex text-xs text-nowrap ${transactionType === 'incoming' ? 'justify-end' : 'justify-start'}`
			)}
		>
			{time}
		</p>
