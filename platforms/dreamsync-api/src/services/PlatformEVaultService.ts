...
export class PlatformEVaultService {
    ...
    /**
     * Check if DreamSync platform eVault already exists
     */
    async checkPlatformEVaultExists(): Promise<boolean> {
        ...
        const existingMapping = await mappingRepository.findOne({
            where: { localUserId: "dreamsync-platform" },
        });
        return !!existingMapping;
    }

    /**
     * Create eVault for DreamSync platform (one-time setup)
     */
    async createPlatformEVault(): Promise<{
        ...
        if (exists) {
            throw new Error("Platform eVault already exists for DreamSync");
        }
        ...
        try {
            ...
            mapping.localUserId = "dreamsync-platform";
            ...
            mapping.userProfileData = {
                ...
                displayName: "DreamSync Platform",
                description:
