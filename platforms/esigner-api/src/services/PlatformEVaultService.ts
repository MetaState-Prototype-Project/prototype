...
export class PlatformEVaultService {
    ...
    /**
     * Check if eSigner platform eVault already exists
     */
    async checkPlatformEVaultExists(): Promise<boolean> {
        ...
        const existingMapping = await mappingRepository.findOne({
            where: { localUserId: "esigner-platform" },
        });
        return !!existingMapping;
    }

    /**
     * Create eVault for eSigner platform (one-time setup)
     */
    async createPlatformEVault(): Promise<{
        ...
        if (exists) {
            throw new Error("Platform eVault already exists for eSigner");
        }
        ...
        try {
            ...
            mapping.localUserId = "esigner-platform";
