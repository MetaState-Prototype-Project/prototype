<script lang="ts">
	...
	async function handleAutoLogin(
		...
		try {
			const response = await fetch(`${PUBLIC_ESIGNER_BASE_URL}/api/auth`, {
				method: 'POST',
	...
	onMount(async () => {
		...
		function watchEventStream(id: string) {
			const sseUrl = new URL(`/api/auth/sessions/${id}`, PUBLIC_ESIGNER_BASE_URL).toString();
...
<div class="flex min-h-screen w-full flex-col items-center justify-center p-4">
	<div class="mb-5 flex flex-col items-center gap-2 text-center">
		...
		<h1 class="text-3xl font-bold text-gray-900">eSigner</h1>
		<p class="text-gray-600">Sign documents with your eID Wallet</p>
	</div>
	...
	>
		...
		<p class="w-full rounded-md bg-white/60 p-4 text-center text-xs leading-4 text-black/40">
			You are entering eSigner - a document signing platform built on the Web 3.0 Data Space
			(W3DS) architecture. Sign documents securely with your eID Wallet.
		</p>
	</div>
</div>
