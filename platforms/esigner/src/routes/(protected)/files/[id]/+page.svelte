<script lang="ts">
    ...
    function checkIfUserSigned() {
        ...
        hasUserSigned = $signatures.some(
            (sig) => sig.fileSigneeId === userInvitation.id,
        );
    }
    ...
    async function handleSign() {
        try {
            ...
            toast.error(
                "Failed to create signing session. Make sure you have a pending invitation.",
            );
        }
    }
    ...
    function watchSigningSession(sessionId: string) {
        ...
        const sseUrl = new URL(
            `/api/signatures/session/${sessionId}`,
            baseUrl,
        ).toString();
            ...
            const proof = {
                proof: {
                    ...
                    summary: {
                        ...
                        signedSignees: invitations.filter(
                            (inv) => inv.status === "signed",
                        ).length,
                        pendingSignees: invitations.filter(
...
<main class="min-h-[calc(100vh-5rem)] flex flex-col overflow-hidden">
    ...
    >
        <a
            href="/files"
            class="inline-flex items-center gap-2 text-blue-600 hover:text-blue-700 text-sm font-medium mb-2"
        >
            <svg
                class="w-4 h-4"
                fill="none"
    ...
    <div class="flex-1 flex flex-col lg:flex-row overflow-hidden">
        ...
        {:else if file}
            ...
            <div
                class="hidden lg:flex lg:flex-[0.7] bg-gray-100 overflow-auto items-center justify-center p-4 sm:p-8"
            >
                {#if previewUrl}
                    {#if file.mimeType?.startsWith("image/")}
                        <div
                            class="max-w-full max-h-full shadow-2xl rounded-lg overflow-hidden bg-white"
                        >
                            <img
                                src={previewUrl}
                                alt={file.name}
                    ...
                    {:else if file.mimeType === "application/pdf"}
                        <div
                            class="w-full h-full max-h-[calc(100vh-12rem)] shadow-2xl rounded-lg overflow-hidden bg-white"
                        >
                            <iframe
                                src={previewUrl}
                    ...
                    {:else}
                        <div
                            class="bg-white rounded-lg shadow-lg p-6 sm:p-12 text-center max-w-md"
                        >
            ...
            >
                <div class="p-4 sm:p-6 space-y-6 min-w-0">
                    ...
                    <div class="border-t border-gray-200 pt-6">
                        ...
                        <div class="space-y-2 text-sm">
                            ...
                            <div class="flex justify-between gap-2">
                                ...
                                >
                                <span
                                    class="text-gray-900 font-medium break-all text-right text-xs sm:text-sm"
                                    >{getMimeTypeDisplayLabel(
                                        file.mimeType || "",
                                    )}</span
                                >
                            </div>
                    ...
                    <div class="border-t border-gray-200 pt-6">
                        <h2 class="text-sm font-semibold text-gray-900 mb-3">
                            Signees <span class="text-gray-500 font-normal"
                            ...
                            <div class="space-y-2">
                                {#each getCombinedSignees() as signee}
                                    ...
                                    >
                                        ...
                                        {#if signee.hasSigned}
                                            <span
                                                class="text-green-600 text-xs font-semibold flex-shrink-0"
                                                >✓ Signed</span
                                            >
                                        {:else if signee.status === "declined"}
                                            <span
                                                class="px-2 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-700 flex-shrink-0"
                                            >
                                                Declined
                                            </span>
                                        {:else}
                                            <span
                                                class="px-2 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-700 flex-shrink-0"
                                            >
                                                Pending
                                            </span>
                                        {/if}
        ...
        >
            ...
            <div class="space-y-3">
                ...
                <button
                    ...
                    class="w-full px-4 py-3 rounded-lg transition-colors text-sm font-medium flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed {allPartiesSigned()
                        ? 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                        : 'bg-gray-50 text-gray-400'}"
                    title={allPartiesSigned()
                        ? ""
                        : "All parties must sign before downloading proof"}
                >
                    <svg
                        class="w-5 h-5"
                        fill="none"
...
{#if showSignModal && signingSession}
    ...
    >
        ...
        >
            ...
            <p class="text-gray-600 mb-6">
                {isMobileDevice()
                    ? "Click the button below to open your eID Wallet and sign"
