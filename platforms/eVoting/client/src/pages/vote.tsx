...
import { Vote as VoteIcon, ArrowLeft, Eye, UserX, CheckCircle, Clock, Users, BarChart3 } from "lucide-react";
...
export default function Vote() {
  ...
  const isVotingAllowed = selectedPoll?.isActive && 
    (!selectedPoll?.deadline || new Date() < new Date(selectedPoll.deadline));
  ...
  const { data: voteStatus } = useQuery({
    queryKey: ["/api/polls", pollId, "vote-status"],
    enabled: !!pollId,
  });

  const { data: resultsData } = useQuery({
    queryKey: ["/api/polls", pollId, "results"],
    enabled: !!pollId && voteStatus?.hasVoted === true,
  });
  ...
  const submitVoteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      // Stay on the same page - it will now show results
    },
    onError: (error: any) => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      <div className="flex items-center justify-between">
        ...
        <Badge variant={selectedPoll.mode === "public" ? "default" : "secondary"}>
      ...
      <div className="card p-8">
        <div className="text-center mb-8">
          ...
          {selectedPoll.deadline && timeRemaining && (
            <div className="mt-3 flex justify-center">
              <Badge 
                variant={timeRemaining.includes("ended") ? "destructive" : "default"}
        ...
        {voteStatus?.hasVoted === true ? (
              ...
              <div className="space-y-3">
                {resultsData?.results.map((option) => {
                  const percentage = resultsData.totalVotes > 0 
                    ? ((option.votes || 0) / resultsData.totalVotes * 100).toFixed(1)
                    : 0;
                  const isUserChoice = option.id === voteStatus.vote?.optionId;
                  const isLeading = resultsData.results.every(r => option.votes >= r.votes);
              ...
              <div className="flex items-center">
                ...
                <div>
                  <p className="text-sm font-medium text-green-900">
                    You voted for: {selectedPoll.options.find(opt => opt.id === voteStatus.vote?.optionId)?.text}
                  </p>
                ...
                <div className="bg-white p-4 rounded-lg border">
                  <div className="flex items-center">
                    ...
                    <div className="ml-4">
                      ...
                      <p className="text-2xl font-bold text-gray-900">{resultsData?.totalVotes || 0}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white p-4 rounded-lg border col-span-2 md:col-span-1">
                  <div className="flex items-center">
                    ...
                    <div className="ml-4">
                      ...
                      <Badge variant={selectedPoll?.isActive ? "success" : "warning"} className="text-lg px-4 py-2">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-6">
                ...
                <div className="space-y-3">
                  {selectedPoll.options.map((option) => {
                    const percentage = selectedPoll.totalVotes > 0 
                      ? ((option.votes || 0) / selectedPoll.totalVotes * 100).toFixed(1)
                      : 0;
                    
                    return (
                      <div key={option.id} className="p-4 rounded-lg border bg-gray-50 border-gray-200">
                        <div className="flex justify-between items-center mb-2">
              ...
              {selectedPoll?.mode === 'public' && selectedPoll.totalVotes > 0 && (
                <div>
                  <h3 className="text-lg font-semibold text-gray-900 mb-4 flex items-center">
          ...
          <div className="space-y-6">
            <div>
              <h3 className="text-lg font-semibold text-gray-900 mb-4">Select your choice:</h3>
              <RadioGroup 
                    ...
                    <div key={option.id} className="flex items-center space-x-3">
                      ...
                      <Label 
                        ...
                        className={`text-base flex-1 py-2 ${
                          isVotingAllowed ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'
                        }`}
                      >
                        {option.text}
                      </Label>
        ...
        {selectedPoll?.mode === 'public' && resultsData?.voterDetails && resultsData.voterDetails.length > 0 && (
          <div className="mt-8 pt-6 border-t border-gray-200">
            ...
            <div className="space-y-2">
              {resultsData.voterDetails.map((voter, index) => (
                <div key={index} className="flex items-center p-3 bg-gray-50 rounded-lg">
                  <img 
                    src={voter.profileImageUrl || '/default-avatar.png'} 
                    alt={voter.firstName || 'Voter'}
                    className="w-8 h-8 rounded-full mr-3"
                    onError={(e) => {
                      e.currentTarget.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(voter.firstName || 'U')}&background=dc2626&color=fff`;
                    }}
                  />
                  <div className="flex-1">
                    <span className="font-medium text-gray-900 block">
                      {voter.firstName && voter.lastName 
                        ? `${voter.firstName} ${voter.lastName}`
                        : voter.email || 'Anonymous'
                      }
                    </span>
                    <span className="text-sm text-gray-500">
                      Voted for: {selectedPoll.options.find(opt => opt.id === voter.optionId)?.text}
                    </span>
                  </div>
                </div>
              ))}
