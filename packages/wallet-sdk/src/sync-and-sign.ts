...
export async function syncPublicKeyToEvault(
  ...
  const { evaultUrl, eName, cryptoAdapter, keyId, token } = options;
  const base = evaultUrl.replace(/\/$/, "");
  const publicKey = await cryptoAdapter.getPublicKey(keyId);
...
export async function signPayload(
  ...
  const { cryptoAdapter, keyId, payload } = options;
  return cryptoAdapter.sign(keyId, payload);
}
