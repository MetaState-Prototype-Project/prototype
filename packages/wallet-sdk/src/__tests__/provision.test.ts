...
describe("provision", () => {
  ...
  it("uses optional namespace and verificationId", async () => {
    ...
    const provisionCall = fetchCall.find(([url]) => url === `${provisionerUrl}/provision`);
  ...
  it("throws when registry entropy fails", async () => {
    ...
    await expect(
      provision({ cryptoAdapter: adapter, registryUrl, provisionerUrl })
    ).rejects.toThrow(/Registry entropy failed/);
  });
